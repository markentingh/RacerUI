                            ...
                            if (car.Skins == null || car.Skins.Count == 0){
                                ...
                                foreach(var skinFolder in skinFolders){
                                    ...
                                    if(File.Exists(skinPath)){
                                        ...
                                        try
                                        {
                                            var skinData = JsonSerializer.Deserialize<UISkin>(skinJson);
                                            if(skinData != null)
                                            {
                                                skin.Name = skinData.Name != "" ? skinData.Name : skin.Path;
                                                skin.Number = skinData.Number;

                                                //find driver by name, or create a driver if neccessary
                                                if (skinData.DriverName != "")
                                                {
                                                    var driver = SQL.DriversRepository.GetByName(skinData.DriverName);
                                                    if (driver == null)
                                                    {
                                                        driver = new Driver();
                                                        driver.Name = skinData.DriverName;
                                                        driver.Id = SQL.DriversRepository.Add(driver);
                                                    }
                                                    skin.DriverId = driver.Id;
                                                }
